...
export const $availablePickupStores = createStore([
  "warehouse A",
  "warehouse B"
]);
...
export const $isDeliveryEmpty = $deliveryAddress.map(
...
const $isCouirerValid = combine(
  $isAddressValid,
  $isZipCodeValid,
  (isAddress, isZip) => isAddress && isZip
);

const $isPostalValid = combine(
  $isAddressValid,
  $isZipCodeValid,
  (isAddress, isZip) => isAddress && isZip
);
...
$deliveryType.on(
  deliveryTypeChanged.map(validateDeliveryType),
  (_, deliveryType) => deliveryType
);
...
function debug(units: any) {
  for (const name in units) {
    const unit = units[name];
    unit.watch((payload: any) => console.log(name, payload));
  }
}
